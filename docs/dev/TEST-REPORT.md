...
2026-09-13：按用户要求将完整源码恢复到默认 `main` 分支，保留简洁 README 与游戏机图片。源码、测试、构建脚本和详细文档在同一分支；`node_modules/`、`dist/`、`artifacts/` 以及本地 `AGENTS.md`、`CHANGELOG.md` 继续排除。`gh-pages` 仍作为在线安装页部署分支。

本轮实测：`npm run check` 通过（类型检查、128/128 测试、生产构建）。运行代码与恢复前的 `source` 分支一致，本次只调整分支与文档，未重复执行 E2E。

## 历史记录：仓库拆分为“分发 + 源码”（已被上述结构取代）
