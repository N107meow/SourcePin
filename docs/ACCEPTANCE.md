...
预期：右下角只出现一个绿色掌机；书签直接进入拾取；安装页首次体验和扩展首次使用才显示引导，说明 Lite、中文、手动录制默认值。点击“开始选择”后引导立即关闭。首次 Esc 清空选择并关闭面板，第二次 Esc 退出；再次唤起不会叠加第二个掌机。
...
在干净的浏览器环境打开安装页，首次点“唤起 SourcePin”应显示引导。直接按 Esc 关闭，再按一次退出；再次体验、刷新安装页或打开同源新标签后，引导不再显示。将书签拖入书签栏后，在不同网站唤起均直接进入选择状态。扩展首次显示引导后，关闭、刷新及切换网站也不再显示。清除对应存储会重置安装页/扩展的首次记录。
