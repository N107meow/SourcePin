...
GitHub 自动生成的 “Source code” 压缩包是该版本标签对应的完整源码，用于开发与自行构建，不包含构建产物；最新源码在 `main` 分支。需要时运行 `npm ci && npm run check`，或 `npm run delivery` 直接生成安装包目录。只想使用工具的话，上面那个 ZIP 就够了。
...
[MIT](https://github.com/N107meow/SourcePin/blob/main/LICENSE)。导出内容（网页文本、图片、Logo）的权利仍归原站/原作者。
