...
export function nodeCss(node: NodeSnapshot): string {
  ...
  const rule=(suffix: string,styles: Record<string,string>)=>`.${node.key}${suffix}{${Object.entries(styles).map(([property,value])=>`${property}:${value};`).join('')}}`;
...
export function serialize(root: Element,snapshots: Map<Element,NodeSnapshot>,maxBytes: number): {html:string;truncatedText:number;skeletonBytes:number} {
  ...
  const visit=(element: Element)=>{
    ...
    if(element.shadowRoot){
      ...
      const shadowCss=[...snapshots].filter(([el])=>el.getRootNode()===element.shadowRoot).map(([,snapshot])=>nodeCss(snapshot)).join('');
      if(shadowCss)parts.push(`<style>${safeEmbeddedCss(shadowCss)}</style>`);
