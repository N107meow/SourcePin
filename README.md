...
用户可感知的功能变化记录在 [`CHANGELOG.md`](./CHANGELOG.md)，与 git 标记（`v0.1.0`、`v0.1.1`、`v0.1.2`）口径一致。
...
项目已经提供 `dist/sourcepin-0.1.2-chrome.zip` 和 `dist/extension`：

1. 解压 `dist/sourcepin-0.1.2-chrome.zip`；也可以直接使用交付目录中的 `dist/extension`。
...
导出确认每次唤起后只完整出现一次，不属于使用引导。首次复制、下载或截图会显示完整确认（个人信息计数、LLM 第三方处理提示、权利声明）；之后同一次唤起内的导出不再弹窗，改为在屏幕底部显示一行小字「已确认 · 声明」，点击可随时重新打开完整声明（只读，不再询问）。
...
- 当前模式常驻显示在机身下方（`LITE` / `PRO`），颜色只是辅助信号；机身下方的 Switch 仅在点击绿色图标后出现，用于切换。
- 「截取组件」「截取当前视口」需要 Chrome 扩展版。书签版与安装页里这两个按钮为禁用态，并在面板内一行和 `title` 中说明原因。Lite 下「开始录制」会一步切到 Pro 并开始录制；「追加当前视口」在未选择元素时会明确提示。
- 最多同时选择 10 个组件；达到上限时再 Shift 点击会提示上限，而不是静默丢弃。Shift 多选提示在首次悬停标签中出现一次。
...
解压 ZIP 后打开 `page.html`。文件包含 doctype、html/head、内联 scoped CSS 和指向净化后原 URL 的 base。**图片不下载也不内联**：每个图片位置放一个尺寸取自快照本身的可解码 SVG 占位，`srcset` 会被移除（否则浏览器会按它去请求原站），原候选仍完整保存在 `structure.json`。因此离线页保留版面与图片位置，但显示的是占位而非像素，体积也不随图片大小增长。多份快照分别输出 `page-2.html` 等文件；`structure.json` 保存结构与审计，`assets.json` 列出被标记的图片位置（`mode: marked-not-fetched`），`report.md` 保留全部报告章节并链接实现参考。
