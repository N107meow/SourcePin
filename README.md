...
用户可感知的功能变化记录在 [`CHANGELOG.md`](./CHANGELOG.md)，与 git 标记（`v0.1.0`、`v0.1.1`）口径一致。
...
项目已经提供 `dist/sourcepin-0.1.1-chrome.zip` 和 `dist/extension`：

1. 解压 `dist/sourcepin-0.1.1-chrome.zip`；也可以直接使用交付目录中的 `dist/extension`。
...
书签栏图标：书签版在网页上唤起时会自行把页面图标设成方形 SourcePin 图标（用 canvas 绘制，严格 CSP 页面同样生效）。安装时已存在的旧书签不会自动更新，删掉再拖一次即可。
