...
// Dismissing the tour and leaving the tool are separate presses now, so the
// helper presses until the inspector is actually gone.
async function pressUntilGone(page){
 for(let attempt=0;attempt<4;attempt++){
  if(await page.locator('[data-sourcepin-root]').count()===0)return;
...
test('bookmarklet never repeats installation guidance across pages or origins',async()=>{
 ...
 for(const url of ['https://one.test/','https://one.test/next','https://two.test/']){
  ...
  await pressUntilGone(page);await page.evaluate(code=>(0,eval)(code),bookmark);await page.locator('[data-sourcepin-root]').waitFor();assert.equal(await guide(page).isVisible(),false);
 }await page.close();
});
test('preview guidance is remembered on first display, including dismissal before Start, reload and a new tab',async()=>{
 ...
 await pressUntilGone(page);await page.evaluate(()=>window.startPreview());assert.equal(await guide(page).isVisible(),false);
...
test('blocked preview storage still remembers guidance across activations in the current page',async()=>{
 const page=await fixture();await page.goto('https://blocked.test');await page.evaluate(()=>{Object.defineProperty(window,'localStorage',{get(){throw new DOMException('Blocked','SecurityError');}});});await page.addScriptTag({content:preview});await page.evaluate(()=>window.startPreview());assert.equal(await guide(page).isVisible(),true);await pressUntilGone(page);await page.evaluate(()=>window.startPreview());assert.equal(await guide(page).isVisible(),false);await page.close();
});
